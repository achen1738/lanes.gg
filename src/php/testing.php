...
const API_KEY = 'RGAPI-e1fd228a-6681-4a77-9b88-b8ed6747d2be';
// Create connection
...
try {
    ...
    if (is_array($matchList) || is_object($matchList)) {
        if (isset($matchList['matches'])) {
            // echo json_encode(new ArrayValue($matchList), JSON_PRETTY_PRINT);
            for ($j = 0; $j < $endIndex; $j++) {
                ...
                // $match = $api->getMatch($gameID, false);
                // $bigArray = array();
    
                // // // Iterate through all the summoners and including their name and info in the array
                // for ($k = 0; $k < 10; $k++) {
                //     $data = array();
                //     $data["summonerName"] = $match['participantIdentities'][$k]['player']['summonerName'];
                //     $data["champName"] = $dDragonData[$match['participants'][$k]['championId']]['name'];
                //     $data["info"] = $match['participants'][$k];
                //     $data["image"] = $dDragonData[$match['participants'][$k]['championId']]['image'];
                //     array_push($bigArray, $data);
                // }

                // $bigArray["matchID"] = $match['gameId'];
                // $bigArray["queueId"] = $queueTypes[$match['queueId']];
                // $bigArray["time"] = timeAgo($match['gameCreation'], $match['gameDuration']);
                // $bigArray["duration"] = duration($match['gameDuration'])[0];
                // $bigArray["minutes"] = duration($match['gameDuration'])[1];
                // // Push the array containing all the details of a match into the final array which will contain all matches
                // // echo json_encode($bigArray);
                // array_push($matches, $bigArray);
            }
            // echo ($var['matches']);
            // echo $matchList['matches'][0]['gameId'];
        }
    }
