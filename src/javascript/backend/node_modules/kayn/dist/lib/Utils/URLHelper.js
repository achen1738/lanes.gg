...
const createRequestURL = (platformID, serviceName, endpoint, version) => `https://${platformID}.${API_HOST}/lol/${serviceName}/v${version}/${endpoint}`;

const getURLWithKey = (platformID, serviceName, endpoint, key, version) => {
    const requestURL = createRequestURL(platformID, serviceName, endpoint, version);
...
const getURLWithQuery = (region, serviceName, endpoint, queryArray, isTournament, version) => createRequestURL(isTournament ? 'americas' : region, serviceName, endpoint, version) + getQueryParamString(queryArray);
