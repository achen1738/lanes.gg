        ...
        ajaxRunes.done(function(runesJSON) {
            ajaxSpells.done(function(spellsJSON) {
                ...
                for (var j = 0; j < length; j++) {
                    ...
                    if (j == length - 1) {
                        lastID = match.matchID;
                    }
                }
                var mGames = document.createElement('div');
                $(mGames).click(function() {
                    mGames.style.pointerEvents = 'none';
                    var promise = moreGames(lastID, name);
                    promise.then((data) => {
                        var newMatches = JSON.parse(data);
                        length = newMatches.matches.length;
                        for (var k = 0; k < length; k++) {
                            var match = newMatches.matches[k];
                            var matchDiv = document.createElement('div');
                            $(matchDiv).addClass("matchItem");
                            $(matchDiv).attr('data-match-id', match.matchID);
                            $(matchDiv).appendTo('.summonerMatches > .Content > .matchList');
                            ...
                            if (newMatches.lastIndex < 98) {
                                console.log('do i get in here');
                                mGames.style.pointerEvents = 'auto';
                            }
                        }
                    })
                });
                var span = document.createElement('span');
                $(span).addClass('moreText').html('Show More');
                $(mGames).addClass('moreGames').append(span).appendTo('.summonerMatches > .Content');
