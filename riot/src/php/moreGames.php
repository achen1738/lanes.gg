...
$inputName = isset($_POST["username"]) ? $_POST["username"] : "thot commander";
$lastMatchId = isset($_POST["lastMatchID"]) ? $_POST["lastMatchID"] : "2847898172";
$userName = str_replace("+", " ", $inputName);
// echo $userName . '<br>' . $lastMatchId;
// echo 'daddy';
try {	
    $mh = curl_multi_init();
    $finalData = array();
    ...
    if (is_array($matchList) || is_object($matchList)) {
        if (isset($matchList['matches'])) {
            $matchesLength = count($matchList['matches']);
            for ($j = 0; $j < $matchesLength; $j++) {
                if ($matchList['matches'][$j]['gameId'] == $lastMatchId) {
                    while ($counter < $endIndex && $j + $counter < 100) {
                        $counter += 1;
                        $timestamp = $matchList['matches'][$j + $counter]['timestamp'];
                        if (daysAgo($timestamp)) {
                            break;
                        }
                        $gameID = $matchList['matches'][$j + $counter]['gameId'];
                        $gameURL = API_URL_MATCH_3 . 'matches/' . $gameID . "?api_key=" . API_KEY;
                        // echo 'daddy';
                        // echo $gameURL . '<br>';
                        $curl_array[$counter - 1] = curl_init($gameURL);
                        // echo ($counter - 1) . '<br>';
                        curl_setopt($curl_array[$counter - 1], CURLOPT_SSL_VERIFYPEER, false);
                        curl_setopt($curl_array[$counter - 1], CURLOPT_RETURNTRANSFER, true);
                        curl_setopt($curl_array[$counter - 1], CURLOPT_HTTPHEADER, 0);
                        curl_multi_add_handle($mh, $curl_array[$counter - 1]);
                    }
                    $finalData['lastIndex'] = $j + $endIndex;
                    break;
                }
            }
        }
