...
async function main(name) {
  ...
  try {
    ...
    if (res.recordset.length == 0) {
      ...
      await req
        .input("accID", sql.VarChar(150), summonerInfoRequest.accountId)
      ...
      summonerResults = await getSummonerInfo(
        res.recordset[0].summonerID,
        conn
      );
...
async function checkResults(name, conn) {
  ...
  var res0 = await req
    .input("sumID", sql.VarChar(150), res.recordset[0].summonerID)
    .query(`SELECT * FROM leagues WHERE summonerID = @sumID`);

  var res1 = req
    .input("accID", sql.VarChar(150), res.recordset[0].accountID)
    .query(
      `SELECT DISTINCT matchID FROM games where accountID = @accID ORDER BY matchID DESC`
    );
...
async function getMatches(matchList, conn, accID) {
  ...
  for (i = 0; i < 20; i++) {
    ...
    const res = await req.query(
      `SELECT * FROM gameInfo WHERE matchID = ${currentGame}`
    );
    if (res.recordset.length === 0) {
...
function insertLeague(league, conn, sumID) {
  var req = new sql.Request(conn);
  return req
    .input("name", sql.VarChar(50), league.leagueName)
...
function insertMatchID(conn, accID, match) {
  ...
  return subReq
    .input("role", sql.VarChar(20), match.role)
...
function insertMatchInfo(
  ...
  return req
    .input("matchID", sql.BigInt, gameId)
